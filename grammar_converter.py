...
def convert_grammar(grammar):
    ...
    for i in range(len(grammar)):
        if len(grammar[i]) > 2:
            for item in grammar[i]:
                if item and item[0] == "'":
                    grammar[i].remove(item)
                    grammar[i].append("T" + item[1:-1])
                    new_rule = ["T" + item[1:-1], item]
                    if new_rule not in grammar:
                        grammar.append(["T" + item[1:-1], item])
    for rule in grammar:
        new_rules = []
...
grammar = read_grammar("input_grammer.txt")
